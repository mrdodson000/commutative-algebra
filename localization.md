...
Let $\varphi : R \to S$ be a homomorphism of rings such that $\varphi (U)$ are units in $S$. Then, the elements $\varphi (r) \varphi (u)^{-1}$ satisfy the same relations that define the equivalence classes $r/u$ in $R[U^{-1}]$. That is, if there exists $v \in U$ such that $v(u'r - ur') = 0$, then $\varphi (r) \varphi (u)^{-1} = \varphi (r') \varphi (u')^{-1}$. Thus, $\varphi$ factors through the natural map $R \to R[U^{-1}]$ where the induced map $\varphi^*$ is given by $r / u \mapsto \varphi (r) \varphi (u)^{-1}$. This should give us some confidence that we have defined $R[U^{-1}]$ correctly, because this result tells us that we have adjoined inverses in the freest possible way.

## Ideal Theory in the Local Ring
Let $R$ be a ring and $U$ be a multiplicatively closed subset.
**Proposition:** Let $\varphi : R \to R[U^{-1}]$ be the natural map. Then we have:
  a) For any ideal $I \subset R[U^{-1}]$, $I = \varphi^{-1}(I)R[U^{-1}]$. Thus, the map $I \mapsto \varphi^{-1}(I)$ is an injection of the set of ideals of $R[U^{-1}]$ into the ideals of $R$. Moreover, this map preserves inclusions, intersections, and takes prime ideals to prime ideals.
  b) An ideal $J \subset R$ is of the form $\varphi^{-1}(I)$ for some $I \subset R[U^{-1}]$ if and only if $J = \varphi^{-1}(JR[U^{-1}])$. Moreover, this is the case if and only if each element $u \in U$ is not a zero divisor mod $J$, in the sense that if $r \in R$ and $ru \in J$, then $r \in J$. These two results together tell us that $\varphi$ induces a bijection between the prime ideals of $R[U^{-1}]$ and the prime ideals $I$ of $R$ such that $I \cap U = \varnothing$.

*Proof.* $I = \varphi^{-1}(I) R[U^{-1}]$ since if $r/u \in I$, $r / 1 \in I$, so we see that $\varphi^{-1}(I)$ generates $I$ over $R[U^{-1}]$. Thus, we have that $I \mapsto \varphi^{-1}(I)$ is an injection from the ideals of $R[U^{-1}]$ to the ideals of $R$. The preservation of inclusions and intersections follows from the standard properties of set maps. That this map takes prime ideals to prime ideals can be seen in the following way: $\varphi$ gives a natural injection $s : R / \varphi^{-1}(I) \to R[U^{-1}] / I$, since if $r / 1 \in I$, we must have $r \in \varphi^{-1}(I)$ so the kernel is trivial. Now, suppose that $I$ is prime. Then, $R[U^{-1}] / I$ is an integral domain. Since $s$ is injective, $R / \varphi^{-1}(I)$ is isomorphic to its image, which must also be an integral domain. Thus $\varphi^{-1}(I)$ is also prime. Note also that this argument holds for ring homomorphisms $\varphi$.

Suppose that $J = \varphi^{-1}(I)$. Then, we have $I = JR[U^{-1}]$, so $J = \varphi^{-1}(JR[U^{-1}])$. Now, recall from above that we have an injection $s : R / J \to R[U^{-1}] / I$. Since the elements of $U$ are units in $R[U^{-1}] / I$, they may not be zero divisors in $R / J$. This completes one direction. Now, suppose that each element $u \in U$ is not a zero divisor mod $J$. It is relatively clear that $J \subset \varphi^{-1}(JR[U^{-1}])$ since if $j \in J$ then $j / 1 \in JR[U^{-1}]$. Now, suppose that $r \in \varphi^{-1}(JR[U^{-1}])$. Then, there is some $j \in J$ such that $r / 1 = j / u$ for some $u \in U$. Thus, by definition, there exists $v \in U$ such that $vur = vj$. Since $u, v$ are not zero divisors mod $J$, we must have $r \in J$. Thus, $J = \varphi^{-1}(JR[U^{-1}])$ as desired.

**Corollary:** The localization of a Noetherian ring is Noetherian.

*Proof.* Let $R$ be a Noetherian ring and $I$ be an ideal of $R[U^{-1}]$. From above, we have that there exists an ideal $J \in R$ such that $I = JR[U^{-1}]$. We also noted above that $J$ generates $I$. Since $R$ is Noetherian, $J$ is finitely generated. Thus, $I$ is finitely generated also. 
